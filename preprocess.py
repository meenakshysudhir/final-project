...
CTD_TREATS = os.path.join(DATA_DIR, "CTD_curated_chemicals_diseases.csv.gz")
SIDER_FILE = os.path.join(DATA_DIR, "meddra_all_se.tsv.gz")
HETIONET_FILE = os.path.join(DATA_DIR, "hetionet-v1.0-edges.sif.gz")
...
def clean_hetionet_compound(x):

    if isinstance(x, str) and "Compound::" in x:
        return x.split("::")[1]

    return None


def clean_hetionet_disease(x):

    if isinstance(x, str) and "Disease::" in x:
        return x.split("::")[1]
...
hetionet.columns = ["source", "relation", "target"]

print("Hetionet edges:", len(hetionet))
print("Relations:", hetionet["relation"].unique())


# Extract treatment edges
hetionet_treats = hetionet[hetionet["relation"] == "treats"]

hetionet_treats["chem"] = hetionet_treats["source"].apply(clean_hetionet_compound)
hetionet_treats["disease"] = hetionet_treats["target"].apply(clean_hetionet_disease)
...
all_chems = sorted(
    set(ctd["ChemicalID"])
    | set(hetionet_treats["chem"].dropna())
)

all_diseases = sorted(
    set(ctd["DiseaseID"])
    | set(hetionet_treats["disease"].dropna())
)
...
# Hetionet edges
print("Adding Hetionet treatment edges...")

for _, row in hetionet_treats.iterrows():
